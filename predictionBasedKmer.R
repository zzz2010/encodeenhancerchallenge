...
###make prediction
testfgMat=read.table("data/LBNLtest.mm10.fa.smat_8mer.gz",header=F)
testfgMat2=read.table("data/LBNLtest.mm10.fa.5merPair.vistasel.smat.gz",header=F)
...
rownames(prediction)<-read.table("data/LBNLtest.mm10.fa.name",stringsAsFactors=F)[,1]

write.table(prediction,file="PredictionUsingKmer.txt",sep="\t",quote=F)
