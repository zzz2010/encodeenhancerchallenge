...
pseudocount = 0
number_filename = ""
selkmer_fn="../data/.fa.5merPair.vistasel.smat.gz.name"
...
while (len(sys.argv) > 2):
  ...
  if (next_arg == "-revcomp"):
    ...
    sys.argv = sys.argv[1:]
  elif (next_arg == "-kmerfile"):
    selkmer_fn = sys.argv[0]
    sys.argv = sys.argv[1:]
...
kmer_id2=dict()
#load kmer defined
for line in open(selkmer_fn):
	comps=line.strip().split()
	kmer=comps[0]+comps[1]
	kmer_id2[kmer]=int(comps[2])
...
i_sequence = 1
outFn=fasta_filename+"."+str(kmerLen)+"merPair.vistasel.smat.gz"
...
while (id != ""):
  ...
  if len(resultList) % 10000==0 and len(resultList) !=0:
	sigCutoff=math.pow(4,math.log(totalSeqLen,4)-kmerLen*2)*30*5
