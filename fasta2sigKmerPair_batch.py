...
##############################
#modified by Zhizhuo Zhang 14 July 2015
#############################
import sys
import math
import gc
...
##############################################################################
# look pair of k-mer, with different gap
...
def make_sequence_vector (sequence,
    ...
    gaplen=30
    k=k_values[len(k_values)-1]
    # Iterate along the sequence.
    for g in range(1,30):
        seq_length = len(sequence)- g - k + 1
	    ...
	    kmer2= sequence[(i_seq+g) : (i_seq+g+ k)]
		...
		rev_kmer2= find_revcomp(kmer2, revcomp_dictionary)
                if (cmp(kmer, rev_kmer) > 0):
                    kmer = rev_kmer
		if (cmp(kmer2, rev_kmer2) > 0):
		    kmer2= rev_kmer2
            ...
            if (mismatch != 0):
                ...
                for i_kmer in range(0, k*2+1): #total length should 2k
                    for letter in alphabet:
                        ...
                        if (kmer[i_kmer:i_kmer+1] != letter):
                            ...
                            if (kmer_counts[bin_num].has_key(neighbor)):
                                kmer_counts[bin_num][neighbor] += 1
                            else:
                                kmer_counts[bin_num][neighbor] = 1
...
if revcomp:
	fn+=".revcomp"
fn+=".txt"
featNameOutf=open(fn,'w')
for i_bin in range(1, num_bins+1):
  for kmer in kmer_list:
    if (num_bins > 1):
      featNameOutf.write("%s-%d\n" % (kmer, i_bin))
    else:
      featNameOutf.write("%s\n" % kmer)
featNameOutf.close()
...
i_sequence = 1
outFn=fasta_filename+"."+str(kmerLen)+"merPair.smat.gz"
outf=gzip.open(outFn,'wb')
resultList_I=0
kmer_id2=dict()
kI2=0
while (id != ""):
  ...
  sys.stdout.write(id+"\n")
  j=-1
  resultList.append(vector[0])
  if len(resultList) % 10000==0 and len(resultList) !=0:
	sigCutoff=math.pow(4,math.log(totalSeqLen,4)-(kmerLen*2-mismatch))*30*1.96
	print "sigCutoff",sigCutoff
	for i in range(len(resultList)):
	        result=resultList[i]
	        for k,v in result.iteritems():
	                if k not in kmer_count or kmer_count[k]<sigCutoff:
	                        continue
			#only use first batch to select the significant k-mer
			if resultList_I>0 and k not in kmer_id2:
				continue
	                if k not in kmer_id2:
	                        kmer_id2[k]=kI2
	                        kI2+=1
	                j=kmer_id2[k]
	                outf.write(str(resultList_I+i)+"\t"+str(j)+"\t"+str(v)+"\n")
  	resultList_I+=len(resultList)
	#reset all the statistic
	totalSeqLen=0
	proc = psutil.Process(os.getpid())
	mem0 = proc.get_memory_info().rss
        print "before GC:",mem0
	resultList=None
        gc.collect()
  ...
  [id, sequence, numbers] = read_sequence_and_numbers(fasta_file,
                                                      ...
                                                      number_file)
  i_sequence += 1
  totalSeqLen+=len(sequence)
# Close the file.
fasta_file.close()
sigCutoff=i_sequence*0.1 #math.pow(4,math.log(totalSeqLen,4)-(kmerLen*2-mismatch))*30*1.96
print "sigCutoff",sigCutoff
for i in range(len(resultList)): 
	result=resultList[i]
	for k,v in result.iteritems():
		if kmer_count[k]<sigCutoff:
			continue
                #only use first batch to select the significant k-mer
                if resultList_I>0 and k not in kmer_id2:
                        continue
		elif k not in kmer_id2:
			kmer_id2[k]=kI2
			kI2+=1
		j=kmer_id2[k]
		outf.write(str(resultList_I+i)+"\t"+str(j)+"\t"+str(v)+"\n")
...
outf2=open(outFn+".name",'w')
for k in kmer_id2:
	km1=k[:kmerLen]
	km2=k[kmerLen:]
	outf2.write(km1+"\t"+km2+"\t"+str(kmer_id2[k])+"\n")
outf2.close()
# Local variables:
# mode: python
# py-indent-offset: 2
